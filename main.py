...
from datasets.aar_dataset import Aar
...
def main():
    ...
    use_cuda = True
    if use_cuda:
        os.environ['CUDA_VISIBLE_DEVICES'] = '1'  # TODO: add to config e.g. 0,1,2,3
    ...
    assert dataset == 'ucf24' or dataset == 'jhmdb21' or dataset == 'ava' or dataset == 'aar', 'invalid dataset'
    ...
    if dataset == 'ava':
        ...
        train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=cfg.TRAIN.BATCH_SIZE, shuffle=True,
                                                   num_workers=cfg.DATA_LOADER.NUM_WORKERS, drop_last=True,
                                                   pin_memory=True)

        test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=cfg.TRAIN.BATCH_SIZE, shuffle=False,
                                                  num_workers=cfg.DATA_LOADER.NUM_WORKERS, drop_last=False,
                                                  pin_memory=True)
        ...
        train_dataset = Aar(cfg, split='train', only_detection=False)
        test_dataset = Aar(cfg, split='val', only_detection=False)
        ...
        train = getattr(sys.modules[__name__], 'train_aar')
        test = getattr(sys.modules[__name__], 'test_aar')
