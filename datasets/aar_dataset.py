...
from datasets import aar_helper, cv2_transform
...
class Aar(torch.utils.data.Dataset):
    ...
    def _load_data(self, cfg):
        ...
        ) = aar_helper.load_image_lists(cfg, is_train=(self._split == "train"))

        # Loading annotations for boxes and labels.
        # boxes_and_labels: {'<video_name>': {<frame_num>: a list of [box_i, box_i_labels]} }
        boxes_and_labels = aar_helper.load_boxes_and_labels(
        ...
        ) = aar_helper.get_keyframe_data(boxes_and_labels)

        # Calculate the number of used boxes.
        self._num_boxes_used = aar_helper.get_num_boxes_used(
            self._keyframe_indices, self._keyframe_boxes_and_labels
        )

        self._max_objs = aar_helper.get_max_objs(
            self._keyframe_indices, self._keyframe_boxes_and_labels
        )
        self.print_summary()
    ...
    def __getitem__(self, idx):
        ...
        # frame-1和frame_idx-1 应该改成 frame和frame_idx 否则读取frame=0的标签时，帧索引self._image_paths[video_idx][-1]实际上是列表最后一张
        ...
        # 这里的frame_idx - 1 也改成frame_idx
        keyframe_info = self._image_paths[video_idx][frame_idx]
